...
まず、`-` を1個送信します。  
`-` は受信状態のリセットの指示であり、`-` を受信したら受信状態を初期状態に戻します。  
`-` は、スイッチの状態の送信を開始する際、(初回だけではなく) 毎回送信します。
...
なっていなければ、エラー状態に入り、`-` を受信するまで `-` 以外のデータを無視します。

古いプロトコルでは、空白 (ASCII 0x20) を受信状態のリセットに用いていました。  
古いプロトコルに対応する場合は、空白も `-` と同様に扱います。

## なでしこ3用プラグイン

以下のようにすることで、[なでしこ3](https://nadesi.com/)でスイッチの情報を受信できます。

```
!「https://mikecat.github.io/switch_as_keyboard/nako3_sak.js」を取り込む。
スイッチ受信した時には
  スイッチ状態を表示。
ここまで。
```
...
### スイッチ受信時 (すいっちじゅしんしたとき)

引数：FUNCで

スイッチの状態を受信し終わった際に実行するコールバック関数を追加します。  
関数を呼び出す際、第1引数にスイッチの状態を表す配列を渡します。  
この配列は、「スイッチ状態」で取得できる配列と同じです。

### スイッチ状態 (すいっちじょうたい)

引数：なし

前回受信したスイッチの状態を表す配列を返します。  
スイッチの状態を受信していない場合は、空の配列を返します。

スイッチの状態は、「最初」のスイッチから「最後」のスイッチの順に、1要素に1個のスイッチの情報を真偽値で格納します。  
`false` が「OFF」を、`true` が「ON」を表します。
